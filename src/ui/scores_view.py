...
class ScoresView:
    def __init__(self, root, service, show_new_game_view, quit, screen_width, screen_height):
        self._root = root
        self._quit = quit
    ...
    def _initialize_score_list(self):
        ...
        i = 2
        for score in scores:
            #label_text = str(score[0])+str(score[1])+str(score[2])+str(score[3])
            #label_text = f"{score[0]:{pad}} {score[1]:{pad}}{score[2]:{pad}}{score[3]:{pad}}"
            #label = ttk.Label(master=self._frame_b, text=label_text)
            #label.grid(column = 0, padx=pad)
