...
class SaveView:
    """Save View näkymä
    """
    def __init__(self, root, service, show_game_view, show_new_game_view, show_load_view, show_scores_view, quit, screen_width, screen_height):
        """luokan konstruktori

        Args:
            root: tkinter root
            service: GameService, pelin rajapinta
            show_game_view: asettaa pelinäkymän
            show_new_game_view: asettaa new_game näkymän
            show_load_view: asettaa load_game näkymän
            show_scores_view: asettaa scores näkymän
    ...
    def _initialize_save_text_box(self):
        ...
        lbl_scores = ttk.Label(master = self._frame_b, text="Save", font=('Cambria 25'))
        ...
        save_button = ttk.Button(
            master=self._frame_b, text="Save", command=lambda: self._handle_save(cbx_file_name.get()))
        save_button.grid(row=2, column=2, sticky='w', padx=0)

        
    def _handle_save(self, filename):
        if self._service.number_of_players < 1:
            self._show_error("File not saved")
        else:
            self._service.save(filename)
